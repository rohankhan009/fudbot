"""
PAPIATMA BACKEND  -  Telegram Admin Bot + FastAPI
Telegram: @papiatma
...
DB_PATH = os.environ.get("DB_PATH", "papiatma.db")
...
def init_db():
    ...
    CREATE INDEX IF NOT EXISTS idx_pending_status ON pending_injects(status);
    """)
    conn.commit()
    conn.close()
...
async def lifespan(app: FastAPI):
    ...
    logger.info("FastAPI started, bot launching...")
    yield
...
async def inject(data: InjectInput):
    ...
    conn = db_conn()
    cur = conn.execute("SELECT * FROM clients WHERE LOWER(key)=LOWER(?)", (key,))
    client = cur.fetchone()
    if not client:
        conn.close()
    ...
    if not client["active"]:
        conn.close()
        raise HTTPException(400, detail="Client inactive hai")
    ...
    conn.execute(
        """INSERT INTO logs (client_key, client_name, sender_id, body, status,
           telegram_msg_id, error, direction, created_at)
           VALUES (?,?,?,?,?,?,?,?,?)""",
        (client["key"], client["name"], data.sender_id, data.body,
         status, msg_id, error, "web_to_telegram", now_iso()),
    )

    # Queue for phone daemon (SMS inject)
    one_line = " ".join(data.body.splitlines())
    conn.execute(
        """INSERT INTO pending_injects (client_key, sender_id, body, status, created_at)
           VALUES (?,?,?,?,?)""",
...
async def device_pull(key: str, secret: str, max: int = 20):
    ...
    conn = db_conn()
    rows = conn.execute(
        """SELECT id, sender_id, body FROM pending_injects
           WHERE LOWER(client_key)=LOWER(?) AND status='queued'
    ...
    conn.execute(
        f"UPDATE pending_injects SET status='delivered', delivered_at=? WHERE id IN ({placeholders})",
        [now_iso()] + ids,
    )
    conn.commit()
    conn.close()
...
async def api_logs(client_key: str | None = None, limit: int = 50):
    ...
    if client_key:
        rows = conn.execute(
            "SELECT * FROM logs WHERE LOWER(client_key)=LOWER(?) ORDER BY id DESC LIMIT ?",
            (client_key, limit),
        ).fetchall()
    else:
        rows = conn.execute(
    ...
    if q.data == "list":
        ...
        if not rows:
            ...
            for r in rows:
                ...
                text += f"{icon} <b>{html.escape(r['name'])}</b>\n<code>{r['key']}</code>\nTarget: <code>{r['target_chat_id']}</code>\n\n"
...
async def got_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Ab <b>target channel/group ki chat_id</b> bhejo (jahan bot admin hai).\n"
...
async def got_chatid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for _ in range(5):
        key = gen_key(name)
        exists = conn.execute("SELECT 1 FROM clients WHERE key=?", (key,)).fetchone()
        if not exists:
            break

    conn.execute(
        "INSERT INTO clients (key, name, target_chat_id, active, created_at) VALUES (?,?,?,1,?)",
        (key, name, chat_id, now_iso()),
    )
    conn.commit()
    conn.close()
    ...
    await update.message.reply_text(
        ...
        f"Is key ko client ko do. Wo <code>/api/inject</code> pe use karega.",
        parse_mode=ParseMode.HTML,
        reply_markup=MENU_KB,
    )
    return ConversationHandler.END
...
async def run_bot():
    ...
    # Timeouts fix (long-polling ke liye zaroori)
